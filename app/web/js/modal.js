            ...
            $('#modal-universe').modal('show')
                ...
                .load($(this).attr('value'), function(response, status, xhr) {
                    if (status == "error") {
                        ...
                        '<div class="pt-2"><button type="button" class="btn btn-secondary float-right" data-dismiss="modal"><i class="fa fa-arrow-left"></i> Close</button></div>'+
                        '' );
                        /* $('#modalContent').html(''+
                        '<div class="mt-2 mb-6 alert text-danger">'+
                            '<big><b class="alert-text">'+xhr.status + '</b> ' + xhr.statusText +'</big>'+
                            '<br><span class="text-muted">'+msg+'</span>'+
                        '</div>'+
